...
void draw()
{
  ...
  text(levelStore,width/3.5,height/25);
  text(fruitCount2,width/1.65,height/25);
  text(fruitNeeded,width/1.05,height/25);
  ...
  else if(levelStore == 4)
  {
    ...
    text("press SPACE to start new game", width/2, height/1.8);
    if(keyPressed&&key==' ')
    {
      reset();
    }
